...
- All function exported by the plug-in are called by the host in the **UI Thread** with the exception of:
  - IAudioProcessor→process: which could be called in an **Audio Thread** (sometime refered as realtime/processing thread), avoid any memory allocation!
  - IAudioProcessor→setProcessing: which could be called in an **Audio Thread**, avoid any memory allocation!
